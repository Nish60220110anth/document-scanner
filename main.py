...
contours, hierarchy = cv2.findContours(
    imgEdged, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)  # find all contours
cv2.drawContours(imgContours, contours, -1, (0, 255, 0),
                 5)  # draw detected contours
...
if biggest.size != 0:
    ...
    cv2.drawContours(imgBigContour, biggest, -1,
                     (0, 255, 0), 20)  # mark biggest contour
    ...
    imgAdaptive = cv2.adaptiveThreshold(imgWarpGray, 255, 1, 1, 7, 2)
    ...
    imageArray = ([img, imgGray, imgEdged, imgContours],
                  [imgBigContour, imgWarpColored, imgWarpGray, imgAdaptive])
